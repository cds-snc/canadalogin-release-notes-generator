...
This dev container provides the read-only tooling used by the weekly release-notes workflow.
...
The container starts without an SSH agent. SSH is only needed when the HTTPS clone fallback in the weekly-release-notes skill is exercised.
