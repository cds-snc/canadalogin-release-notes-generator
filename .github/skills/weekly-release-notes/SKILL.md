...
2. For `canadalogin-migration` and `canadalogin-user-selfservice-webapp`, for each environment file that exists:
   ...
   - Show Test, Staging, and Production for every repository. For each environment, state the deployed version and date when it changed during the reporting period, or say `No deployment recorded this week` when it did not. Include release notes only for environments that changed.
...
*Summary*
One or two upbeat, concise sentences highlighting the week's meaningful deployments, promotions, and user-facing progress.
