...
1. Treat the three repositories and the IBM wiki as subdirectories of the current workspace. If one is missing or is not a Git repository, clone it directly into that path with complete history, trying SSH first and HTTPS second. Clone the wiki to `./gc-signin-ibm-configuration.wiki`. Do not use a workspace-parent or home-directory fallback.
