...
1. Parse the user's request into `start`, `end`, `cadence`, and `environment` values before gathering repository evidence. Named arguments are supported in any order: `from=YYYY-MM-DD`, `to=YYYY-MM-DD`, `start=YYYY-MM-DD`, `end=YYYY-MM-DD`, `cadence=weekly|bi-weekly|monthly`, and `environment=test|staging|prod`. Ordinary phrasing is also supported, including `from YYYY-MM-DD to YYYY-MM-DD`, `YYYY-MM-DD through YYYY-MM-DD`, `for August 2026`, `last week`, and `last month`. Treat `production` as an alias for `prod`. Do not infer an environment from repository names or incidental words in the request.
2. If the user supplies a start and end date, use that inclusive range. A month phrase means the complete calendar month in UTC. `last week` means the previous complete Monday-to-Sunday UTC week; `last month` means the previous complete UTC calendar month. Otherwise use the previous fourteen complete calendar days in UTC. State the selected range in the report.
3. If the user supplies an optional cadence keyword (`weekly`, `bi-weekly`, or `monthly`, case-insensitive), split the requested range into consecutive inclusive sub-ranges of that length and generate one full set of reports per sub-range. Accept cadence in any position, including named form, such as `2026-07-01 to 2026-09-01 bi-weekly` or `from=2026-07-01 to=2026-09-01 cadence=bi-weekly`. Without a cadence keyword, treat the whole range as a single sub-range and generate one set of reports. Sub-ranges are date-shifted from the requested start date: `weekly` = 7 days per sub-range, `bi-weekly` = 14 days per sub-range, `monthly` = one calendar month per sub-range (start-day-shifted; for example, 2026-07-15 → 2026-08-14). The final sub-range may be shorter when the requested range does not divide evenly; clamp its end to the requested (or clamped) end date. Skip a sub-range whose start date is strictly after the effective end date.
4. If the user supplies an optional environment argument (`test`, `staging`, or `prod`, case-insensitive), generate reports only for that environment. Normalize `prod` and `production` to the Production environment and the `production` filename stem. If no environment argument is supplied, generate reports for all three environments. Accept the environment argument with or without a date range or cadence, including named form such as `environment=staging`; do not infer an environment from other wording. If an environment value is supplied outside this set, ask the user to choose `test`, `staging`, or `prod` rather than guessing.
5. If two values conflict, such as a named `environment=prod` and a natural-language request for Test, ask the user to clarify rather than choosing one. If a date phrase is ambiguous or incomplete, ask for a date range rather than guessing.
6. Use the last committed state immediately before each sub-range's start date as that sub-range's comparison baseline. Every sub-range independently answers "what was deployed in this sub-range?" and "what changed compared with what was there before it?". Do not reuse baselines across sub-ranges.
7. If the requested (or default) end date is later than the current UTC calendar date, clamp the effective end date to the current UTC date before splitting into sub-ranges. Use the clamped date for every history lookup, keep the original filename dates for the sub-range that contains the clamped end, and add a Notes bullet in that sub-range's reports stating the end date was clamped.
8. Treat the three repositories and the IBM wiki as sibling directories of the current workspace. If a sibling is missing or is not a Git repository, clone it with `fetch-depth: 0` (complete history). Try `git@github.com:cds-snc/<REPO_NAME>.git` first; fall back to `https://github.com/cds-snc/<REPO_NAME>.git`, using a locally configured `gh` or credential helper when the repository is private. If the workspace parent is read-only (for example `/workspaces/` in a dev container), clone to `$HOME/repos/<REPO_NAME>` — and `$HOME/repos/gc-signin-ibm-configuration.wiki` for the wiki — and use that path for all reads. Fail only if neither SSH nor HTTPS succeeds.
9. Use read-only Git and GitHub operations. Never push, commit, tag, create or delete branches, create or update releases, open or modify PRs or issues, dispatch workflows, or modify source files, changelogs, deployment files, or wiki pages. The only files this skill may create or update are the generated Markdown and HTML reports and `index.html` at the workspace root.
